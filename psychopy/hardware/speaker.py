...
class SpeakerDevice(BaseDevice):
    ...
    def __init__(self, index=None, name=None):
        ...
        for thisProfile in ptb.get_devices(device_type=13):
            ...
            if thisProfile['DeviceName'] in (name, index) or (
                index == thisProfile['DeviceIndex'] and name is None
            ):
        ...
        if self.profile is None:
            raise ValueError(
                "No speaker device found with index {}".format(index)
            )
        
        # --- Initialise ptb stream ---
    ...
    def isSameDevice(self, other):
        ...
        return index in (self.index, self.profile['DeviceName'])
